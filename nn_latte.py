...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, t, c):
        shift_msa, scale_msa, gate_msa, gate_mxa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(
            t).chunk(7, dim=1)
        x = x + \
            gate_msa.unsqueeze(
...
class Latte(nn.Module):
    ...
    def __init__(
        ...
        if not latent:
            ...
            self.x_embedder = PatchEmbed(
                input_size, patch_size, 2 + pos_embed_dim, hidden_size, bias=True)
        else:
            self.x_embedder = PatchEmbed(
        ...
        self.register_buffer(
            "frame_embed",
            torch.zeros(1, num_frame_tokens, hidden_size)
        )
    ...
    def initialize_weights(self):
        ...
        frame_embed = get_2d_sincos_pos_embed(
            self.temp_embed.shape[-1], int(math.sqrt(self.num_frame_tokens)))
        self.frame_embed.data.copy_(
            torch.from_numpy(frame_embed).float().unsqueeze(0))
    ...
    def forward(self,
        ...
        timestep_spatial = repeat(
            t, 'n d -> (n c) d', c=self.temp_embed.shape[1])
