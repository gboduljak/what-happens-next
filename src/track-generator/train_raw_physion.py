...
def sample_latte_raw(
    ...
    query_points = repeat(
        pixel_center_grid(256, 256)[
            2::4, 2::4][::trajectory_stride, ::trajectory_stride, :],
        "h w d -> b h w d",
        b=batch_size
    ).to(device)
    query_points = normalize_points(query_points, 256, 256)
...
def generate_samples(
    ...
    gt_motion = visualizer.visualize(
        torch.zeros((batch_size, 30, 3, 256, 256)),
        rearrange(
        ...
        for batch_idx, batch in enumerate(batch_loader):
            ...
            if should_restore:
                ...
                if step == cfg.training.num_steps_per_epoch:
                    should_restore = False
                    break
            frames, frame_features, tracks = batch
            loss = train_step(
                model,
                fm,
                Batch(frames, frame_features, tracks),
                device,
                trajectory_stride=cfg.model.nn.trajectory_stride,
                use_amp=True
            ).mean()
            ...
            # Save periodically
            if rank == 0 and (step % (cfg.training.checkpoint_every_steps) == 0):
                save(
            ...
            if rank == 0 and (step % (cfg.training.sample_every_steps) == 0):
                ...
                # Sample
                model.eval()
                frames, frame_features, tracks = train_batch
                ...
                frames, frame_features, tracks = val_batch
